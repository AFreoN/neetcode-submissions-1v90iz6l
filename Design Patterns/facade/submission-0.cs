...
public class DriveThruFacade {
    ...
    public Food takeOrder(string orderContents, bool takeOut) {
        ...
        Order newOrder = new Order(orderContents, takeOut);
        Food food = new Food(orderContents);
        if (takeOut) food = new PackagedFood(food);

        cashier.takeOrder(orderContents, takeOut);
        chef.prepareFood(newOrder);
        kitchenStaff.packageOrder(food);
        return food;
    }
}
